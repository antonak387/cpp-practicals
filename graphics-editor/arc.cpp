...
{
    if ((new_startA >=-360) && (new_startA<=360)) //устанавливаем диапазон
    {
        startA = new_startA;//подставляем значение начального угла
        return true;
    }
    else
    {
...
{
    if ((new_spanA >=-360)&&(new_spanA<=360)) //устанавливаем диапазон
    {
        spanA = new_spanA;
        return true;
    }
...
void arc::GetAngels(int &cur_x, int &cur_y)//возвращение начального угла и длины угла
{
    cur_x=startA;
    cur_y=spanA;
}
